...
func main() {
	...
	// Custom CA file for HTTPS
	flCAFile := pflag.String("ca-file",
		envString("", "GITSYNC_CA_FILE", "GIT_SYNC_CA_FILE"),
		"the CA certificate file to use for HTTPS (sets GIT_SSL_CAINFO)")
	...
	// Set custom CA file for HTTPS if provided
	if *flCAFile != "" {
		if _, err := os.Stat(*flCAFile); err != nil {
			log.Error(err, "can't access CA file", "file", *flCAFile)
			os.Exit(1)
		}
		if err := os.Setenv("GIT_SSL_CAINFO", *flCAFile); err != nil {
			log.Error(err, "can't set GIT_SSL_CAINFO", "file", *flCAFile)
			os.Exit(1)
		}
		log.V(1).Info("set GIT_SSL_CAINFO for HTTPS", "file", *flCAFile)
	}
